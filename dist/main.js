...
async function main() {
    ...
    if (repCible) {
        ...
        if (repCibleFR)
            await fs.mkdir(repCibleFR, { recursive: true });
        if (repCibleEURCPNotices)
            await fs.mkdir(repCibleEURCPNotices, { recursive: true }); // Création du nouveau répertoire
        if (repCibleEU)
            await fs.mkdir(repCibleEU, { recursive: true });
        if (repCibleRCP)
