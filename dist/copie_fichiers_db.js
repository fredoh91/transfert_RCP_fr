...
export async function logCopieFichier(row) {
    ...
    if (hasTypeDocument) {
      ...
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, row.rep_fichier_source, row.nom_fichier_source, row.rep_fichier_cible, row.nom_fichier_cible, row.code_cis, row.code_atc, row.date_copie_rep_tempo, row.resultat_copie_rep_tempo, row.date_copie_sftp, row.resultat_copie_sftp, row.id_batch, row.type_document);
    }
    else {
        await db.run(`INSERT INTO liste_fichiers_copies (
        rep_fichier_source, nom_fichier_source, rep_fichier_cible, nom_fichier_cible, code_cis, code_atc,
        date_copie_rep_tempo, resultat_copie_rep_tempo, date_copie_sftp, resultat_copie_sftp, id_batch
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, row.rep_fichier_source, row.nom_fichier_source, row.rep_fichier_cible, row.nom_fichier_cible, row.code_cis, row.code_atc, row.date_copie_rep_tempo, row.resultat_copie_rep_tempo, row.date_copie_sftp, row.resultat_copie_sftp, row.id_batch);
    }
}
